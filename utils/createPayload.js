 ...
 */
export const getQueryInfo = (totalRows, totalRowsPerPage = 10) => {
  return {
    total: totalRows,
...
export function createPayload(data = {}, errors) {
  return errors
    ? { ok: false, errors: extractErrors(errors) }
    : { ok: true, ...data };
}

export function createPaginatedPayload(data, errors) {
  const { count, documents } = data;
  const queryInfo = getQueryInfo(count);

  return createPayload({ documents, queryInfo }, errors);
}
